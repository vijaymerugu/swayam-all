...
@Query(value = " select nvl(count(CALL_SUBCATEGORY),0) from TBL_TICKET_CENTRE where (AGEING>2 and AGEING<=4)" , nativeQuery = true)
          public int find2_4HoursCount();

// for ageing count 
@Query(value = "select nvl(count(CALL_SUBCATEGORY),0)  from TBL_TICKET_CENTRE where   (AGEING>2 and AGEING<=4)  "
...
@Query(value = " SELECT *  from TBL_TICKET_CENTRE where  (AGEING>2 and AGEING<=4)  and KIOSK_ID IN (SELECT KIOSK_ID FROM TBL_USER_KIOSK_MAPPING  WHERE PF_ID=:pfId) ", nativeQuery = true)
public Page<TicketCentor> findAllTicketCentor4HourAndCMFUser(@Param("pfId") String pfId,Pageable pageable);

@Query(value = "SELECT *  from TBL_TICKET_CENTRE where  (AGEING>2 and AGEING<=4) "
		, nativeQuery = true)
...
@Query(value = " SELECT * from TBL_TICKET_CENTRE where  (AGEING>24) "
		+ " AND KIOSK_ID IN (SELECT KIOSK_ID FROM TBL_USER_KIOSK_MAPPING WHERE PF_ID=:pfId)", nativeQuery = true)
public Page<TicketCentor> findAllTicketCentor3DaysGreaterAndCMFUser(@Param("pfId") String pfId,Pageable pageable);
...
@Query(value="SELECT nvl(count(CALL_SUBCATEGORY),0)  from TBL_TICKET_CENTRE where  AGEING >2 ", nativeQuery=true)
public int findTotalCount();
...
@Query(value = " SELECT nvl(count(CALL_SUBCATEGORY),0)  from TBL_TICKET_CENTRE where  (AGEING>=2 and AGEING<=4)  and KIOSK_ID in (SELECT KIOSK_ID FROM TBL_KIOSK_MASTER WHERE CIRCLE=:circle)", nativeQuery = true)
public int find2_4HoursCount(@Param("circle") String circle) ;
...
@Query(value="SELECT nvl(count(CALL_SUBCATEGORY),0)  from TBL_TICKET_CENTRE where AGEING >2 and KIOSK_ID in (SELECT KIOSK_ID FROM TBL_KIOSK_MASTER WHERE CIRCLE=:circle)", nativeQuery=true)
public int findTotalCount(@Param("circle") String circle);
...
@Query(value = " select * from TBL_TICKET_CENTRE where (AGEING>2 and AGEING<=4) " + 
			" and KIOSK_ID in (SELECT KIOSK_ID FROM TBL_KIOSK_MASTER WHERE CIRCLE=:circle)", nativeQuery = true)
	public Page<TicketCentor> findAllTicketCentor4HourByCircle(@Param("circle") String circle,Pageable pageable);
...
@Query(value = " SELECT *  from TBL_TICKET_CENTRE where (AGEING>2 and AGEING<=4)  AND KIOSK_ID IN (SELECT KIOSK_ID FROM TBL_USER_KIOSK_MAPPING WHERE PF_ID in (:pfId))", nativeQuery = true)
public Page<TicketCentor> findAllTicketCentor4HourAndCMSUser(@Param("pfId") Set<String> pfId,Pageable pageable);
...
//@Query(value = " SELECT  *  from TBL_TICKET_CENTRE where  AGEING between to_date(trunc(sysdate-2/24), 'DD-MM-YY HH24:MI:SS' ) and  to_date(to_char(trunc(sysdate-3),'DD-MM-YY')||' 23:59:59',  'DD-MM-YY HH24:MI:SS') ", nativeQuery = true)
@Query(value=" SELECT  *  from TBL_TICKET_CENTRE where  AGEING >2 ", nativeQuery=true)
public Page<TicketCentor> findAllTicketCentorTotal(Pageable pageable);
...
@Query(value="SELECT nvl(count(CALL_SUBCATEGORY),0)  from TBL_TICKET_CENTRE where  AGEING >2 and KIOSK_ID in (SELECT KIOSK_ID FROM TBL_USER_KIOSK_MAPPING WHERE PF_ID=:pfId)", nativeQuery=true)
public int findTotalCountCMF(@Param("pfId") String pfId);

@Query(value = " SELECT nvl(count(CALL_SUBCATEGORY),0)  from TBL_TICKET_CENTRE where  (AGEING>=2 and AGEING<=4) and KIOSK_ID in (SELECT KIOSK_ID FROM TBL_USER_KIOSK_MAPPING WHERE PF_ID in (:pfId))", nativeQuery = true)
public int find2_4HoursCountCMF(@Param("pfId") Set<String> pfId) ;
...
@Query(value="SELECT  nvl(count(CALL_SUBCATEGORY),0)  from TBL_TICKET_CENTRE where  AGEING >2 and KIOSK_ID in (SELECT KIOSK_ID FROM TBL_USER_KIOSK_MAPPING WHERE PF_ID in (:pfId))", nativeQuery=true)
public int findTotalCountCMF(@Param("pfId") Set<String> pfId);
