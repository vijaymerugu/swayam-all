...
public interface InvoiceCompareRepository extends PagingAndSortingRepository<InvoiceCompare, String>{
	...
	@Query(value = "select s1.RFP_REF_NO,s1.VENDOR,  s2.CRCL_NAME, s2.STAT_DESC, s3.KIOSK_ID, s3.KIOSK_SERIAL_NO,s3.QTR_ID,s3.FIN_YR,s3.INVOICE_AMT,s4.INVO_AMT,ABS(s3.INVOICE_AMT - s4.INVO_AMT) as DIFFERENCE from\r\n" + 
			"                        (select v.PRN_SRN,v.INVO_AMT from TBL_INVOICE_VENDOR v) s4\r\n" + 
			"                        JOIN\r\n" + 
			"                        ((select d.KIOSK_ID,d.KIOSK_SERIAL_NO ,d.QTR_ID,d.FIN_YR," + 
			"						d.CORRECTION_AMT, d.INVOICE_AMT from TBL_INVOICE d where  d.QTR_ID=:quterTimePeriod AND d.FIN_YR=:finacialYear) s3 \r\n" + 
			"								JOIN \r\n" + 
