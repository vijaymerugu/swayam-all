...
public interface BillingPenaltyRepository extends PagingAndSortingRepository<BillingPenaltyEntity, String> {
	...
	@Query(value = 
			"select s1.RFP_REF_NO,s1.VENDOR,  s2.CRCL_NAME, s2.STAT_DESC, s3.KIOSK_ID, s1.KIOSK_SERIAL_NO,s3.TIME_PERIOD,s3.TOTAL_HRS,s3.DOWNTIME,s3.RELAXATION,s3.FINAL_DOWNTIME, "+
					"s3.PENALTY from(select d.KIOSK_ID, d.TIME_PERIOD, d.TOTAL_HRS, d.DOWNTIME, d.RELAXATION, d.FINAL_DOWNTIME, "+
					"d.PENALTY from TBL_DOWNTIME_QTR d where d.time_period=:quterTimePeriod) s3 "+
					"JOIN "+
					...
					"On  s1.BRANCH_CODE = s2.BRANCH_CODE) "+
						...
						countQuery ="select count(s3.KIOSK_ID) from(select d.KIOSK_ID, d.TIME_PERIOD, d.TOTAL_HRS, d.DOWNTIME, d.RELAXATION, d.FINAL_DOWNTIME, "+
								"d.PENALTY from TBL_DOWNTIME_QTR d where d.time_period=:quterTimePeriod) s3 "+
								"JOIN "+
								"((select k.KIOSK_ID, k.BRANCH_CODE ,k.VENDOR,k.KIOSK_SERIAL_NO,k.RFP_REF_NO from TBL_KIOSK_MASTER k where k.VENDOR In " +
								"(select v.VENDOR from TBL_VENDOR_DETAILS v where VEND_ID=:selectedVendorId )) s1 "+
