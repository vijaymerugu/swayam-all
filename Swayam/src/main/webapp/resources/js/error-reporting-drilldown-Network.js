...
app.controller('ErrorReportDrillDownCtrl', ['$scope','$filter','ErrorReportDrillDownService','uiGridConstants', function ($scope, $filter,ErrorReportDrillDownService,uiGridConstants) {
  ...
  var counttype = "NW";     
   ...
   $scope.loadHomeBodyPageForms = function(networkName){
		...
		if(circleName != undefined || networkName != undefined){	
			var str ='td/errorRepoDrillDownModule?circleName='+circleName+'&networkName='+networkName+'&fromDate='+fromDate+'&toDate='+toDate;
	...
	//	alert(code);
		...
		if(circleName != undefined || networkName != undefined){	
			var str ='td/errorRepoNoOfBussFailDrillDownNW?networkCode='+code+'&fromDate='+fromDate+'&toDate='+toDate;
			$("#contentHomeApp").load(str);
		}						
	}
   ...
   $scope.loadHomeBodyPageForms1 = function(code){
		...
		if(circleName != undefined || networkName != undefined){	
			var str ='td/errorRepoNoOfTechFailDrillDownNW?networkCode='+code+'&fromDate='+fromDate+'&toDate='+toDate;
			$("#contentHomeApp").load(str);
		}						
	}
 ...
 $scope.backUser = function()
   {  	
	        ...
	        $("#loading").show();  
	       // alert("i m inside backUser"+fromDate);
	        circleName = "";
	        var str ='td/errorDrillDownNetworkBack?circleName='+circleName+'&networkName='+networkName+'&fromDate='+fromDate+'&toDate='+toDate;
			$("#contentHomeApp").load(str);
			$("#loading").hide();  
   ...
   $scope.gridOptions = {
    ...
    columnDefs: [
      { name: 'name',
      	  ...
      	  cellTemplate: '<div class="ui-grid-cell-contents"><a style="cursor: hand;cursor: pointer;" ng-click="grid.appScope.loadHomeBodyPageForms(row.entity.code)">NET-0{{row.entity.name}}</a></div>'},
