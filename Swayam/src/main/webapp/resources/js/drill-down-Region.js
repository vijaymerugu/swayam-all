...
//   var networkName="";
   ...
   $scope.loadHomeBodyPageForms = function(circleName, networkName, moduleName, regionName){	   
		if(circleName != undefined || networkName != undefined || moduleName != undefined || regionName != undefined){	
			var str ='/td/drillDownBranch?circleName='+circleName+'&networkName=' + networkName +'&moduleName=' + moduleName+'&regionName=' + regionName;
   ...
   $scope.getCountType = function(type){
       ...
       DrillDownService.getUsers(paginationOptions.pageNumber,
			   paginationOptions.pageSize,counttype,circleName,networkName,moduleName).success(function(data){
   ...
   $scope.gridOptions = {
    ...
    columnDefs: [
      { name: 'region',
      	  ...
      	  cellTemplate: '<div class="ui-grid-cell-contents"><a ng-click="grid.appScope.loadHomeBodyPageForms(row.entity.circleCode,row.entity.networkCode,row.entity.moduleCode,row.entity.regionCode)">{{row.entity.region}}</a></div>'
      },
    ...
    onRegisterApi: function(gridApi) {
        ...
        gridApi.pagination.on.paginationChanged($scope, function (newPage, pageSize,counttype) {
          ...
          DrillDownService.getUsers(newPage,pageSize,counttype,circleName,networkName,moduleName).success(function(data){
        	  $scope.gridOptions.data = data.content;
         	  $scope.gridOptions.totalItems = data.totalElements;
          });
        });
